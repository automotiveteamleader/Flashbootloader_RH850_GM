 ...
 *  --------------------------------------------------------------------------------------------------------------------
 *  REVISION HISTORY (KbFbl_apnvFrameOem.h, GM SLP5 and SLP6)
 *  --------------------------------------------------------------------------------------------------------------------
 *  Version    Date        Author  Change Id        Description
 *  --------------------------------------------------------------------------------------------------------------------
 *  05.00.00   2014-01-31  AWh     -                First release
 *  05.00.01   2015-06-12  AWh     -                Update Header Information
 *  05.01.00   2016-01-13  AWh     ESCAN00087444    No changes
 *  05.02.00   2016-03-23  Shs     ESCAN00089101    MISRA related code changes
 *  05.02.01   2016-05-17  AWh     ESCAN00090044    Include fbl_hdr.h instead of fbl_diag.h
 **********************************************************************************************************************/
/***********************************************************************************************************************
 *  --------------------------------------------------------------------------------------------------------------------
 *  REVISION HISTORY
...
/* ##V_CFG_MANAGEMENT ##CQProject : FblKbApi_FrameNv_GM CQComponent : Implementation */
#define FBLKBAPI_FRAMENV_GM_VERSION           0x0502u
#define FBLKBAPI_FRAMENV_GM_RELEASE_VERSION   0x01u

/***********************************************************************************************************************
 *  INCLUDES
 **********************************************************************************************************************/

/* Change include order to have "tNBIDInfo" type */
#include "fbl_hdr.h"
...
/* Choose between function and macro interface */
...
#else
# if defined( FBL_HDR_ENABLE_BASIC_NVM_HANDLING )
#  error "Only Function Interface supported"
# else
#  define ApplFblNVMReadSBATicket(buffer) ApplFblNvReadSBA_ticket(0u, (buffer))
#  define ApplFblNVMWriteSBATicket(buffer) ApplFblNvWriteSBA_ticket(0u, (buffer))
#  define ApplFblNVMReadECUID(buffer) ApplFblNvReadECU_ID(0u, (buffer))
#  define ApplFblNVMWriteECUID(buffer) ApplFblNvWriteECU_ID(0u, (buffer))
#  define ApplFblNVMReadKeyNBID(nbidInfo) ApplFblNvReadKey_NBID(0u, (V_MEMRAM1 vuint8 V_MEMRAM2 V_MEMRAM3 *)&(nbidInfo)->stored)
#  define ApplFblNVMWriteKeyNBID(nbidInfo) ApplFblNvWriteKey_NBID(0u, (V_MEMRAM1 vuint8 V_MEMRAM2 V_MEMRAM3 *)&(nbidInfo)->received)
# endif
# if ( FBL_MTAB_NO_OF_BLOCKS > 1 )
  /* In case of Multi processor configuration implement AppNBID handling in function. */
# else
#  define ApplFblNVMReadAppNBID(nbidInfo) ApplFblNvReadApp_NBID(0u, (V_MEMRAM1 vuint8 V_MEMRAM2 V_MEMRAM3 *)&(nbidInfo)->stored)
#  define ApplFblNVMWriteAppNBID(nbidInfo) ApplFblNvWriteApp_NBID(0u, (V_MEMRAM1 vuint8 V_MEMRAM2 V_MEMRAM3 *)&(nbidInfo)->received)
# endif
#endif
...
tFblResult ApplFblReadTpTargetAddr(V_MEMRAM1 vuint8 V_MEMRAM2 V_MEMRAM3 * buffer);
tFblResult ApplFblWriteTpTargetAddr(V_MEMRAM1 vuint8 V_MEMRAM2 V_MEMRAM3 * buffer);

void ApplFblGetBaseModulePPRegion( vuint8 partId, V_MEMRAM1 IO_PositionType V_MEMRAM2 V_MEMRAM3 * pPresPtnAddr, V_MEMRAM1 IO_SizeType V_MEMRAM2 V_MEMRAM3 * pPresPtnLen );
...
void ApplFblReadSBATicket( V_MEMRAM1 vuint8 V_MEMRAM2 V_MEMRAM3 * const buffer );
...
#if (kFblPresencePatternSize != 2)
# error "Implementation of ApplFblChkPSIState requires kFblPresencePatternSize of 2"
#endif
