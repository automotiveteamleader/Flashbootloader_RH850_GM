...
#else
   /* AUTOSAR FEE Read */
   retVal = ApplFblNvRead( 0x8u, 0u, buffer, 822u);
#endif
   return retVal;
}
...
#else
   /* AUTOSAR FEE Write */
   retVal = ApplFblNvWrite( 0x8u, buffer);
#endif
   return retVal;
}
...
#else
   /* AUTOSAR FEE Read */
   retVal = ApplFblNvRead( 0xAu, 0u, buffer, 16u);
#endif
...
#else
   /* AUTOSAR FEE Write */
   retVal = ApplFblNvWrite( 0xAu, buffer);
#endif
...
tFblResult ApplFblNVMReadKeyNBID( V_MEMRAM1 tNBIDInfo V_MEMRAM2 V_MEMRAM3 * const keyNBIDInfo )
{
   ...
   /* AUTOSAR FEE Read */
   retValue = ApplFblNvRead( 0xEu, 0u, (vuint8*)&keyValue, 2u);

   keyNBIDInfo->stored = keyValue;
...
tFblResult ApplFblNVMWriteKeyNBID( const V_MEMRAM1 tNBIDInfo V_MEMRAM2 V_MEMRAM3 * const keyNBIDInfo )
{
   ...
   key = keyNBIDInfo->received;

   /* AUTOSAR FEE Write */
   return ApplFblNvWrite( 0xEu, (vuint8*)&key);
}
#endif
...
tFblResult ApplFblNVMReadAppNBID( V_MEMRAM1 tNBIDInfo V_MEMRAM2 V_MEMRAM3 * const appNBIDInfo )
{
...
#endif
   {
      ...
      /* AUTOSAR FEE Read */
      retValue = ApplFblNvRead( 0xCu, 0u, (vuint8*)&appValue, 2u);

      appNBIDInfo->stored = appValue;
...
tFblResult ApplFblNVMWriteAppNBID( const V_MEMRAM1 tNBIDInfo V_MEMRAM2 V_MEMRAM3 * const appNBIDInfo )
{
...
#endif
   {
      ...
      app = appNBIDInfo->received;

       /* AUTOSAR FEE Write */
       return ApplFblNvWrite( 0xCu, (vuint8*)&app);
   }
}
#endif
